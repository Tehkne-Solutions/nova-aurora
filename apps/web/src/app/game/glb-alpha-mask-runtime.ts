...
export type AlphaMaskMode = "OPAQUE" | "MASK";

export type AlphaMaskPbrDrawable = NormalMappedPbrDrawable & Readonly<{
  alphaMode: AlphaMaskMode;
  alphaCutoff: number;
}>;

export type AlphaMaskPbrModel = Readonly<{
  drawables: readonly AlphaMaskPbrDrawable[];
  ...
  materialModel: "pbr-alpha-mask-v1";
}>;
...
function materialAlpha(document: JsonObject, materialIndex: number | null): Readonly<{ alphaMode: AlphaMaskMode; alphaCutoff: number }> {
  ...
  if (mode === "BLEND") throw new Error(`materials[${materialIndex}].alphaMode=BLEND ainda não pertence ao perfil 23.15; exige ordenação/composição transparente própria.`);
  if (mode !== "OPAQUE" && mode !== "MASK") throw new Error(`materials[${materialIndex}].alphaMode inválido: ${mode}.`);
  if (mode === "OPAQUE") return { alphaMode: "OPAQUE", alphaCutoff: 0.5 };
...
export function parseAlphaMaskPbrGlb(buffer: ArrayBuffer): AlphaMaskPbrModel {
  const { document, bin } = parseGlb(buffer);
  const base = parseNormalMappedPbrGlb(buildSanitizedGlb(document, bin));
  const profiles = primitiveProfiles(document);
  if (profiles.length !== base.drawables.length) throw new Error("Ordem de primitivas não corresponde ao runtime alpha-mask.");

  let alphaMaskedMaterials = 0;
  let opaqueMaterials = 0;
  const drawables = base.drawables.map((drawable, primitiveIndex): AlphaMaskPbrDrawable => {
