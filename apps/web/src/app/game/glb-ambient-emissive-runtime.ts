import {
  parseDualTexturePbrGlb,
  type DualTexturePbrDrawable,
  type EmbeddedMaterialTexture
} from "./glb-dual-texture-runtime";

export type AmbientEmissivePbrDrawable = DualTexturePbrDrawable & Readonly<{
  occlusionTexture: EmbeddedMaterialTexture | null;
  occlusionStrength: number;
  emissiveTexture: EmbeddedMaterialTexture | null;
}>;

export type AmbientEmissivePbrModel = Readonly<{
  drawables: readonly AmbientEmissivePbrDrawable[];
  ...
  materialModel: "pbr-lite-ambient-emissive-v3";
}>;
...
function textureDescriptor(document: JsonObject, bin: Uint8Array, textureIndex: number, label: string): EmbeddedMaterialTexture {
  ...
  const texture = textures[index(textureIndex, textures.length, `${label}.index`)]!;
  if (texture.extensions !== undefined) throw new Error(`${label} texture extensions ainda não pertencem ao runtime 23.13.`);
  ...
  if (sampler?.extensions !== undefined) throw new Error("Sampler extensions ainda não pertencem ao runtime 23.13.");
  return {
    textureIndex,
    imageIndex,
...
function textureInfo(
  ...
  bin: Uint8Array,
  label: string
): EmbeddedMaterialTexture | null {
  if (value === undefined) return null;
  const info = object(value, label);
  if (info.extensions !== undefined) throw new Error(`${label}.extensions ainda não pertence ao perfil 23.13.`);
  if (info.texCoord !== undefined && integer(info.texCoord, `${label}.texCoord`) !== 0) throw new Error(`${label}.texCoord precisa ser 0.`);
  return textureDescriptor(document, bin, integer(info.index, `${label}.index`), label);
}

function materialEffects(document: JsonObject, bin: Uint8Array, materialIndex: number | null): MaterialEffects {
  if (materialIndex === null) return { occlusionTexture: null, occlusionStrength: 1, emissiveTexture: null };
  const materials = array(document.materials, "materials").map((value, position) => object(value, `materials[${position}]`));
  const material = materials[materialIndex];
  if (!material) return { occlusionTexture: null, occlusionStrength: 1, emissiveTexture: null };
  const occlusionInfo = material.occlusionTexture === undefined ? null : object(material.occlusionTexture, `materials[${materialIndex}].occlusionTexture`);
  if (occlusionInfo?.extensions !== undefined) throw new Error("occlusionTexture.extensions ainda não pertence ao perfil 23.13.");
  if (occlusionInfo?.texCoord !== undefined && integer(occlusionInfo.texCoord, "occlusionTexture.texCoord") !== 0) throw new Error("occlusionTexture.texCoord precisa ser 0.");
  const occlusionTexture = occlusionInfo
    ? textureDescriptor(document, bin, integer(occlusionInfo.index, "occlusionTexture.index"), `materials[${materialIndex}].occlusionTexture`)
    : null;
  const occlusionStrength = occlusionInfo ? factor(occlusionInfo.strength, 1, "occlusionTexture.strength") : 1;
  const emissiveTexture = textureInfo(material.emissiveTexture, document, bin, `materials[${materialIndex}].emissiveTexture`);
  return { occlusionTexture, occlusionStrength, emissiveTexture };
}

export function parseAmbientEmissivePbrGlb(buffer: ArrayBuffer): AmbientEmissivePbrModel {
  const { document, bin } = parseGlb(buffer);
  const sanitized = buildSanitizedGlb(document, bin);
  const base = parseDualTexturePbrGlb(sanitized);
  const profiles = primitiveProfiles(document);
  if (profiles.length !== base.drawables.length) throw new Error("Ordem de primitivas não corresponde ao runtime ambient-emissive.");

  let occlusionTexturedMaterials = 0;
  let emissiveTexturedMaterials = 0;
  ...
  for (const drawable of base.drawables) {
    ...
    if (drawable.metallicRoughnessTexture) uniqueTextures.set(drawable.metallicRoughnessTexture.textureIndex, drawable.metallicRoughnessTexture.bytes.byteLength);
  }

  const drawables = base.drawables.map((drawable, primitiveIndex): AmbientEmissivePbrDrawable => {
    const primitive = profiles[primitiveIndex]!;
    const effects = materialEffects(document, bin, primitive.materialIndex);
    let texCoords = drawable.texCoords;
    if (effects.occlusionTexture || effects.emissiveTexture) {
      if (primitive.texCoordAccessor === null) throw new Error(`primitive[${primitiveIndex}] usa texture de material sem TEXCOORD_0.`);
      if (!texCoords) {
    ...
    if (effects.occlusionTexture) {
      occlusionTexturedMaterials += 1;
      uniqueTextures.set(effects.occlusionTexture.textureIndex, effects.occlusionTexture.bytes.byteLength);
    }
    if (effects.emissiveTexture) {
      emissiveTexturedMaterials += 1;
      uniqueTextures.set(effects.emissiveTexture.textureIndex, effects.emissiveTexture.bytes.byteLength);
    }
    return { ...drawable, texCoords, ...effects };
  });
  ...
  return {
    ...
    occlusionTexturedMaterials,
    emissiveTexturedMaterials,
