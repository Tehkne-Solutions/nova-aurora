...
async function renderAlphaMaskPbr(canvas: HTMLCanvasElement, drawables: readonly AlphaMaskPbrDrawable[], rotationDegrees: number): Promise<() => void> {
  ...
  if (Number(gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS)) < 5) throw new Error("GPU não oferece cinco texture units para o perfil alpha-mask v8.");
  ...
  const alphaMaskLocation = gl.getUniformLocation(program, "u_alpha_mask");
  const alphaCutoffLocation = gl.getUniformLocation(program, "u_alpha_cutoff");
  ...
  if (
    ...
    || !normalScaleLocation || !alphaMaskLocation || !alphaCutoffLocation || !lightLocation || !cameraLocation
    || !baseTextureLocation || !mrTextureLocation || !occlusionTextureLocation || !emissiveTextureLocation
    || !normalTextureLocation || !hasBaseTextureLocation || !hasMrTextureLocation || !hasOcclusionTextureLocation
    || !hasEmissiveTextureLocation || !hasNormalTextureLocation
  ) {
    gl.deleteProgram(program);
    throw new Error("Shader GLB alpha-mask-v8 incompleto.");
  }
  ...
  try {
    for (const drawable of drawables) {
      ...
      if (!positionBuffer || !normalBuffer) throw new Error("Falha ao criar buffers alpha-mask-v8.");
  ...
  const draw = () => {
    ...
    for (const resource of resources) {
      ...
      gl.uniform1f(alphaMaskLocation, resource.drawable.alphaMode === "MASK" ? 1 : 0);
      gl.uniform1f(alphaCutoffLocation, resource.drawable.alphaCutoff);
...
export function GlbPlacement({ assetUrl, label, rotationYDegrees = 0, current = false }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(assetUrl, { signal: controller.signal })
      ...
      .then(async (buffer) => {
        ...
        const model = parseAlphaMaskPbrGlb(buffer);
        const cleanup = await renderAlphaMaskPbr(canvas, model.drawables, rotationYDegrees);
      ...
      .catch((cause: unknown) => {
        if (controller.signal.aborted) return;
        setError(cause instanceof Error ? cause.message : "Modelo 3D/alpha mask incompatível.");
        setState("error");
      });
  ...
  return (
    <div
      ...
      data-glb-renderer="first-party-webgl-pbr-alpha-mask-v8"
      data-material-state={materialState}
      role="img"
      title={state === "error" ? `${label}: ${error ?? "modelo não suportado"}` : `${label} · PBR + alpha mask cutout`}
    >
      <canvas aria-hidden="true" ref={canvasRef} />
      <span className={styles.badge} aria-hidden="true">GLB · PBR+MASK</span>
      {state === "loading" ? <span className={styles.loading}>materializando recortes 3D…</span> : null}
      {state === "error" ? <span className={styles.fallback}>3D seguro<br />alpha mask indisponível</span> : null}
    </div>
  );
}
