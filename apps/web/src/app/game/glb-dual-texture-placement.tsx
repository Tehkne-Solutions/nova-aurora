...
type GpuDrawable = Readonly<{
  drawable: DualTexturePbrDrawable;
...
function createProgram(gl: WebGLRenderingContext): WebGLProgram {
  ...
  const fragment = compileShader(gl, gl.FRAGMENT_SHADER, `
    ...
    void main(){
      ...
      vec3 diffuse = baseColor * (1.0 - metallic) * (hemisphere + 0.78 * nDotL);
      ...
      vec3 linearColor = min(diffuse + specular + u_emissive + baseColor * 0.035, vec3(1.0));
...
async function renderDualTexturePbr(canvas: HTMLCanvasElement, drawables: readonly DualTexturePbrDrawable[], rotationDegrees: number): Promise<() => void> {
  ...
  if (
    ...
    || !baseColorLocation || !metallicFactorLocation || !roughnessFactorLocation || !emissiveLocation || !lightLocation || !cameraLocation
    || !baseTextureLocation || !mrTextureLocation || !hasBaseTextureLocation || !hasMrTextureLocation
  ) {
    gl.deleteProgram(program);
    throw new Error("Shader GLB dual-texture-v5 incompleto.");
  }
  ...
  try {
    for (const drawable of drawables) {
      ...
      if (!positionBuffer || !normalBuffer) throw new Error("Falha ao criar buffers dual-texture-v5.");
      ...
      resources.push({ drawable, positionBuffer, normalBuffer, texCoordBuffer, baseColorTexture, metallicRoughnessTexture });
    }
  } catch (error) {
...
export function GlbPlacement({ assetUrl, label, rotationYDegrees = 0, current = false }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(assetUrl, { signal: controller.signal })
      ...
      .then(async (buffer) => {
        ...
        const model = parseDualTexturePbrGlb(buffer);
        const cleanup = await renderDualTexturePbr(canvas, model.drawables, rotationYDegrees);
  ...
  return (
    <div
      ...
      data-glb-renderer="first-party-webgl-pbr-dual-texture-v5"
      data-material-state={materialState}
      role="img"
      title={state === "error" ? `${label}: ${error ?? "modelo não suportado"}` : `${label} · PBR baseColor + metallic/roughness`}
    >
      <canvas aria-hidden="true" ref={canvasRef} />
      <span className={styles.badge} aria-hidden="true">GLB · PBR×2</span>
      {state === "loading" ? <span className={styles.loading}>materializando materiais 3D…</span> : null}
