...
import {
  alphaBlendDepth,
  parseAlphaBlendPbrGlb,
  type AlphaBlendPbrDrawable
} from "./glb-alpha-blend-runtime";
...
type ShaderBindings = Readonly<{
  ...
  modelRotationLocation: WebGLUniformLocation;
...
function requiredUniform(gl: WebGLRenderingContext, program: WebGLProgram, name: string): WebGLUniformLocation {
  ...
  if (!location) throw new Error(`Shader GLB alpha-blend-v9 não expôs ${name}.`);
  return location;
}
...
function shaderBindings(gl: WebGLRenderingContext, program: WebGLProgram): ShaderBindings {
  ...
  if (positionLocation < 0 || normalLocation < 0 || tangentLocation < 0 || texCoordLocation < 0) {
    throw new Error("Shader GLB alpha-blend-v9 não expôs todos os atributos.");
  }
  return {
    positionLocation,
    normalLocation,
    ...
    modelRotationLocation: requiredUniform(gl, program, "u_model_rotation"),
...
function bindDrawable(
  gl: WebGLRenderingContext,
  bindings: ShaderBindings,
  resource: GpuDrawable,
...
async function renderAlphaBlendPbr(canvas: HTMLCanvasElement, drawables: readonly AlphaBlendPbrDrawable[], rotationDegrees: number): Promise<() => void> {
  ...
  if (Number(gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS)) < 5) throw new Error("GPU não oferece cinco texture units para o perfil alpha-blend v9.");
  ...
  try {
    for (const drawable of drawables) {
      ...
      if (!positionBuffer || !normalBuffer) throw new Error("Falha ao criar buffers alpha-blend-v9.");
  ...
  const solidResources = resources.filter((resource) => resource.drawable.alphaMode !== "BLEND");
  const transparentResources = resources.filter((resource) => resource.drawable.alphaMode === "BLEND");
  ...
  const draw = () => {
    ...
    gl.uniformMatrix4fv(bindings.viewProjectionLocation, false, new Float32Array(multiply(projection, view)));
    gl.uniformMatrix4fv(bindings.modelLocation, false, new Float32Array(model));
    gl.uniformMatrix3fv(bindings.modelRotationLocation, false, modelRotation);
    gl.uniform3fv(bindings.lightLocation, lightDirection);
    gl.uniform3fv(bindings.cameraLocation, cameraPosition);
    gl.uniform1i(bindings.baseTextureLocation, 0);
    gl.uniform1i(bindings.mrTextureLocation, 1);
    gl.uniform1i(bindings.occlusionTextureLocation, 2);
    gl.uniform1i(bindings.emissiveTextureLocation, 3);
    gl.uniform1i(bindings.normalTextureLocation, 4);

    // Pass 1: sólidos e recortes escrevem profundidade normalmente.
    gl.depthMask(true);
    gl.disable(gl.BLEND);
    for (const resource of solidResources) bindDrawable(gl, bindings, resource, whiteTexture);

    // Pass 2: transparências respeitam o depth buffer sólido, mas não o alteram.
    const sortedTransparent = [...transparentResources].sort(
      (left, right) => alphaBlendDepth(right.drawable, rotationDegrees, camera) - alphaBlendDepth(left.drawable, rotationDegrees, camera)
    );
    if (sortedTransparent.length > 0) {
      gl.depthMask(false);
      gl.enable(gl.BLEND);
      gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
      for (const resource of sortedTransparent) bindDrawable(gl, bindings, resource, whiteTexture);
    }

    // Restaura estado para resize/repaint futuro e para evitar vazamento de estado.
    gl.depthMask(true);
    gl.disable(gl.BLEND);
  };
...
export function GlbPlacement({ assetUrl, label, rotationYDegrees = 0, current = false }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(assetUrl, { signal: controller.signal })
      ...
      .then(async (buffer) => {
        ...
        const model = parseAlphaBlendPbrGlb(buffer);
        const cleanup = await renderAlphaBlendPbr(canvas, model.drawables, rotationYDegrees);
        if (controller.signal.aborted) { cleanup(); return; }
        dispose = cleanup;
        setMaterialState(
          model.alphaBlendedMaterials > 0
            ? "alpha-blend"
            : model.alphaMaskedMaterials > 0
              ? "alpha-mask"
      ...
      .catch((cause: unknown) => {
        if (controller.signal.aborted) return;
        setError(cause instanceof Error ? cause.message : "Modelo 3D/transparência incompatível.");
        setState("error");
      });
  ...
  return (
    <div
      ...
      data-glb-renderer="first-party-webgl-pbr-alpha-blend-v9"
      data-material-state={materialState}
      role="img"
      title={state === "error" ? `${label}: ${error ?? "modelo não suportado"}` : `${label} · PBR + fila transparente`}
    >
      <canvas aria-hidden="true" ref={canvasRef} />
      <span className={styles.badge} aria-hidden="true">GLB · PBR+BLEND</span>
      {state === "loading" ? <span className={styles.loading}>ordenando transparências 3D…</span> : null}
      {state === "error" ? <span className={styles.fallback}>3D seguro<br />transparência indisponível</span> : null}
    </div>
  );
}
