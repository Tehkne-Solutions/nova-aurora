...
function createProgram(gl: WebGLRenderingContext): WebGLProgram {
  ...
  const fragment = compileShader(gl, gl.FRAGMENT_SHADER, `
    ...
    void main(){
      ...
      float hemisphere = 0.30 + 0.16 * (normal.y * 0.5 + 0.5);
      float lighting = min(1.15, hemisphere + diffuse * 0.72);
      vec3 litColor = u_color.rgb * lighting + vec3(0.018, 0.022, 0.026);
...
type GpuDrawable = Readonly<{
  drawable: LitDrawable;
  positionBuffer: WebGLBuffer;
  normalBuffer: WebGLBuffer;
}>;

function renderLit(canvas: HTMLCanvasElement, drawables: readonly LitDrawable[], rotationDegrees: number): () => void {
  ...
  if (positionLocation < 0 || normalLocation < 0 || !mvpLocation || !modelRotationLocation || !colorLocation || !lightLocation) {
    gl.deleteProgram(program);
    throw new Error("Shader GLB lit incompleto.");
  }

  const resources: GpuDrawable[] = drawables.map((drawable) => {
    const positionBuffer = gl.createBuffer();
    const normalBuffer = gl.createBuffer();
    if (!positionBuffer || !normalBuffer) throw new Error("Falha ao criar buffers GLB lit.");
  ...
  const draw = () => {
    ...
    for (const resource of resources) {
      if (resource.drawable.doubleSided) {
...
export function GlbPlacement({ assetUrl, label, rotationYDegrees = 0, current = false }: Props) {
  ...
  const [normalMode, setNormalMode] = useState<"accessor" | "fallback" | null>(null);
  ...
  useEffect(() => {
    ...
    void fetch(assetUrl, { signal: controller.signal })
      ...
      .then((buffer) => {
        ...
        const model = parseLitGlb(buffer);
        dispose = renderLit(canvas, model.drawables, rotationYDegrees);
        setNormalMode(model.fallbackNormalPrimitives > 0 ? "fallback" : "accessor");
        setState("ready");
      })
      .catch((cause: unknown) => {
        if (controller.signal.aborted) return;
  ...
  return (
    <div
      ...
      data-glb-renderer="first-party-webgl-lit-v2"
      data-normal-mode={normalMode ?? "pending"}
      role="img"
      title={state === "error" ? `${label}: ${error ?? "modelo não suportado"}` : `${label} · iluminação 3D`}
    >
      <canvas aria-hidden="true" ref={canvasRef} />
      <span className={styles.badge} aria-hidden="true">GLB · LIT</span>
      {state === "loading" ? <span className={styles.loading}>iluminando 3D…</span> : null}
