...
export class EconomySnapshotService extends EconomyRepositoryBase {
  ...
  async resolveAnomaly(anomalyId:string,resolvedBy:string,reason:string):Promise<EconomyAnomalyView>{
    const normalizedReason=reason.trim();
    if(normalizedReason.length<10)throw new Error("A justificativa deve ter pelo menos 10 caracteres.");
    return this.sql.begin("isolation level serializable",async(tx)=>{
      ...
      const updated=await tx`UPDATE economy_snapshot_anomalies SET resolved_at=now(),resolved_by=${resolvedBy}::uuid,resolution_reason=${normalizedReason} WHERE id=${anomalyId}::uuid AND resolved_at IS NULL RETURNING *`;
      const row=updated[0];if(!row)throw new Error("Anomalia econômica não resolvida.");
      await this.outbox(tx,String(row.snapshot_id),"economy.snapshot.anomaly_resolved",{anomalyId:String(row.id),snapshotId:String(row.snapshot_id),anomalyKey:String(row.anomaly_key),severity:String(row.severity),resolvedBy,reason:normalizedReason});
