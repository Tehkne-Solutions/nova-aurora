  ...
  async summary(): Promise<ReleaseReadinessSummary> {
    const [users, emails, integrity, gates, trust] = await Promise.all([
      this.sql`
        SELECT
          count(*)::int total,
          count(*) FILTER (WHERE public_beta_access='active' AND status='active')::int active_beta,
          count(*) FILTER (WHERE email_verification_required=true OR email_verified_at IS NULL)::int pending_verification,
          count(*) FILTER (WHERE public_beta_access='suspended' OR status<>'active')::int suspended
        FROM users
      `,
      this.sql`
        SELECT
      ...
      this.sql`
        SELECT
          ...
          (SELECT count(*)::int FROM user_risk_profiles WHERE economic_status IN ('restricted','frozen')) restricted_users
      `,
      this.sql`
        SELECT
